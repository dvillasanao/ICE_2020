...
for(i in tablas){
  assign(paste0("df_",i), read.csv(paste0("DENUE/CSV/CSV/conjunto_de_datos/denue_inegi_",i,"_.csv"), fileEncoding = "latin1"))
}
...
save(denue2021, file = "Bases/DENUE/denue2021.RData")
```
...
```{r, eval = FALSE}
load("Bases/CLUES_Estructura_AGEB.RData")
```
...
for(i in tablas){
 assign(paste0("TI_", i), read_sav(paste0("TI_MANZANA/TI_MANZANA_EU_",i,".SAV")))
}
...
# Se guarda la base de datos donde se integran 16 bases de datos
save(MANZANA_EU_2020, file = "Bases/MANZANA_EU_2020.RData")
```

No enfocamos en los indicadores númericos. 

```{r, eval = FALSE}
load("Bases/MANZANA_EU_2020.RData")
```
...
save(EU_mun, file = "Bases/Municipio/EU_mun.RData")
```
...
save(EU_loc, file = "Bases/Localidad/EU_loc.RData")
```
...
save(EU_ageb, file = "Bases/AGEB/EU_ageb.RData")
```
