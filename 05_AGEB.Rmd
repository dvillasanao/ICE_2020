...
```{r}
load("Bases/AGEB/MGN_ageb.Rdata")
```
...
PC_IMU <-  IMU %>%
              select(CVE_AGEB) %>%
               mutate(PC = as.numeric(scale(as.matrix(IMU[,c(2:ncol(IMU))])) %*% 
                 ...
                 as.data.frame()
save(PC_IMU, file = "Output/AGEB/PC_IMU.Rdata")
```

**Resumen**

```{r, echo = FALSE}
load("Output/AGEB/facto_summarize_IMU.Rdata")
tabla %>%
...
load("Bases/AGEB/Indicadores_DENUE.RData")
```
...
save(PC_DENUE, file = "Output/AGEB/PC_DENUE.Rdata")
```
...
saveRDS(pcr_model, "Output/AGEB/pcr_model_denue.rds")
## Caso máximo
pcr_pred_max <- NULL
for(i in 1:11){
...
write.table(pcr_extremos, file = "Output/AGEB/PCR_DENUE.txt", sep = ",")
```
...
```{r}
load("Bases/AGEB/Indicadores_Escuelas_SEP.RData")
...
save(PC_SEP,  file = "Output/AGEB/PC_SEP.Rdata")
```
...
saveRDS(pcr_model, "Output/AGEB/pcr_model_sep.rds")
## Caso máximo
...
write.table(pcr_extremos, file = "Output/AGEB/PCR_SEP.txt", sep = ",")
```
...
```{r}
load("Bases/AGEB/Indicadores_CLUES.RData")
...
save(PC_CLUES,  file = "Output/AGEB/PC_CLUES.Rdata")
```
...
saveRDS(pcr_model, "Output/AGEB/pcr_model_clues.rds")
## Caso máximo
...
write.table(pcr_extremos, file = "Output/AGEB/PCR_CLUES.txt", sep = ",")
```
...
PC_EU <-  EU_ageb %>%
              select(CVE_AGEB) %>%
               mutate(PC = as.numeric(scale(as.matrix(EU_ageb[, c(2:ncol(EU_ageb))])) %*% 
                 ...
                 as.data.frame()
save(PC_EU,  file = "Output/AGEB/PC_EU.Rdata")
```
...
saveRDS(pcr_model, file = "Output/AGEB/pcr_model_eu.rds")
...
write.table(pcr_extremos, file = "Output/AGEB/PCR_EU.txt", sep = ",")
```
...
```{r, echo = FALSE}
pcr_model <- readRDS("Output/AGEB/pcr_model_eu.rds")
```


```{r}
load("Output/AGEB/EU_imputacion.Rdata")
```
         ...
         theme_ipsum_es(grid="X") +
           theme(title = element_text(size = 15, family = "montserrat"),
                  plot.title = element_text(size = 12,family = "montserrat"),
                            plot.subtitle = element_text(size = 10, hjust = 0.5, family = "montserrat"),
...
load("Output/AGEB/PC_DENUE.Rdata")
...
```{r}
load("Output/AGEB/PC_CLUES.Rdata")
...
```{r}
PCR_pred <- read.table("Output/AGEB/PCR_SEP.txt", header = TRUE, sep = ",") %>% 
             rbind(.,read.table("Output/AGEB/PCR_CLUES.txt", header = TRUE, sep = ",")) %>%
              rbind(., read.table("Output/AGEB/PCR_DENUE.txt", header = TRUE, sep = ","))
```
...
save(sd, file = "Output/AGEB/sd.Rdata")
```

```{r, echo = FALSE}
load("Output/AGEB/sd.Rdata")
```
...
save(ICE_2020, file = "Bases/AGEB/ICE_2020.RData")
```
