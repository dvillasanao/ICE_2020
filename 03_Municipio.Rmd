...
Se toma como referencia el marco geoestadístico nacional 2020 a Nivel
Municipal.

```{r, codefolding_show = "hide" }
load("Bases/Municipio/MGN.Rdata")
```
...
**Es importante seleccionar que gráficos se pueden incluir en el
análisis**

Se utilizó como criterio los gráficos de sedimentación y la contriución
de la variables en cada uno de los componentes y la varianza acumulada

Por otro lado se generan dos índices, de los cuales: - Uno se
estandariza para explicarlos a través de descomposición de valores
singulares (`SVD`) de la matriz de datos.

$$\mathbf X = \mathbf U \mathbf S \mathbf V^\top$$ - Segundo, desde el
...
Donde - $\mathbf V$ son los eigenvectors - $\mathbf U$ es la matriz
unitaria - $\mathbf S$ es la traza de los eigenvalues

Las columnas de $\mathbf {US}$ son los pesos de las componentes
principales
...
save(PC_IMM, file = "Output/Municipio/PC_IMM.Rdata")
```
...
tabla %>%
  kable(caption = "Tabla: Resumen de los indicadores de marginación utilizando PCA", 
               digits = 3, 
                format.args = list(font_size = '10px', family = "montserrat"),
                 format = "html", 
                  booktabs = FALSE) %>% 
        kable_classic(full_width = FALSE, html_font = "montserrat", position = "center") %>% 
...
Se anexan los indicadores simples, de acuerdo a la clasificación
económica del DENUE (**01**)

```{r,eval = TRUE}
load("Bases/Municipio/Indicadores_DENUE.RData")
```

Se utiliza la nueva reclasificación de los subsitemas, de los cuales
fueron obtenidos del DENUE
...
```{r, collapse=TRUE}
load("Bases/Subsistemas_DENUE.Rdata")
...
tabla %>% 
  ...
  kable(caption = "Tabla: Dimensiones de cálculo", 
          format.args = list(font_size = '10px', family = "montserrat", big.mark = " "),
            align = 'c',
            format = "html") %>% 
        kable_classic(full_width = FALSE, html_font = "montserrat", position = "center") %>% 
         kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 9) %>%
...
save(PC_DENUE, file = "Output/Municipio/PC_DENUE.Rdata")
```
...
saveRDS(pcr_model, "Output/Municipio/pcr_model_denue.rds")
...
write.table(pcr_extremos, file = "Output/Municipio/PCR_DENUE.txt", sep = ",")
```
...
PC_SEP <-  sep_mun %>%
              select(CVE_MUN) %>%
               mutate(PC = as.numeric(scale(as.matrix(sep_mun[, c(3:11)])) %*% 
                 ...
                 as.data.frame()
save(PC_SEP,  file = "Output/Municipio/PC_SEP.Rdata")
```
...
saveRDS(pcr_model, "Output/Municipio/pcr_model_sep.rds")
...
write.table(pcr_extremos, file = "Output/Municipio/PCR_SEP.txt", sep = ",")
```

### Salud {.unnumbered}

```{r}
load("Bases/Municipio/Indicadores_CLUES.RData")
...
save(PC_CLUES,  file = "Output/Municipio/PC_CLUES.Rdata")
```
...
saveRDS(pcr_model, "Output/Municipio/pcr_model_clues.rds")
...
write.table(pcr_extremos, file = "Output/Municipio/PCR_CLUES.txt", sep = ",")
```
...
```{r}
load("Bases/Municipio/IA_municipio.Rdata") 
```
...
```{r}
load("Output/Municipio/PC_IMM.Rdata")
...
load("Output/Municipio/PC_DENUE.Rdata")
...
```{r}
load("Output/Municipio/PC_SEP.RData")
SEP <- PC_SEP %>%
         rename("Educacion" = "PC")
```
...
```{r}
load("Output/Municipio/PC_CLUES.RData")
CLUES <- PC_CLUES %>%
          rename("Salud" = "PC")
```
...
```{r}
PCR_pred <- read.table("Output/Municipio/PCR_SEP.txt", header = TRUE, sep = ",") %>% 
             rbind(.,read.table("Output/Municipio/PCR_CLUES.txt", header = TRUE, sep = ",")) %>%
              rbind(., read.table("Output/Municipio/PCR_DENUE.txt", header = TRUE, sep = ","))
```
       ...
       geom_boxplot(aes(x = ICE, y = "", color = ICE), fill = "transparent", size = 0.7) + 
        geom_jitter(aes(x = ICE, y = "", color = ICE, fill = ICE), size = 2, alpha = 0.3) + 
         theme_ipsum_rc(grid = "X") +
                     theme(title = element_text(size = 15, family = "montserrat"),
                            plot.title = element_text(size = 15,family = "montserrat"),
                            plot.subtitle = element_text(size = 12, hjust = 0, family = "montserrat"),
                             plot.caption = element_text(size = 9, family = "montserrat"),
                             axis.text.x = element_text(size = 6, family = "montserrat"), 
                             axis.text.y = element_text(size = 6, family = "montserrat"), 
                              axis.title = element_text(size = 10, family = "montserrat"), 
                               axis.title.y = element_text(size = 10, family = "montserrat"), 
...
save(sd, file = "Output/Municipio/sd.Rdata")
```

```{r, echo = FALSE}
load("Output/Municipio/sd.Rdata")
```
...
min.strata %>%
    kable(col.names = c("n", "Error \n estándar", "CV"),
                   digits = 4, 
                    align = 'c',
                    format.args = list(font_size = '9px', family = "montserrat"),
                     format = "html", 
                      booktabs = FALSE) %>% 
            kable_classic(full_width = FALSE, html_font = "montserrat", position = "center") %>% 
             kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 9) %>%
...
save(ICE_2020, file = "Bases/Municipio/ICE_2020.RData")
```
