...
```{r, echo=TRUE}
load("Bases/Localidad/MGN.Rdata")
```
...
```{r, eval = FALSE}
load("Bases/Localidad/IML.Rdata")
...
PC_IML <-  IML %>%
             select(CVE_LOC) %>%
               mutate(PC = as.numeric(scale(as.matrix(IML[,c(2:ncol(IML))])) %*% 
                 ...
                 as.data.frame()
save(PC_IML, file = "Output/Localidad/PC_IML.Rdata")
```
...
```{r, echo = FALSE}
load("Output/Localidad/facto_summarize_IML.Rdata")
tabla %>%
...
load("Bases/Localidad/Indicadores_DENUE.RData")
```
...
save(PC_DENUE, file = "Output/Localidad/PC_DENUE.Rdata")
```
...
saveRDS(pcr_model, "Output/Localidad/pcr_model_denue.rds")
...
write.table(pcr_extremos, file = "Output/Localidad/PCR_DENUE.txt", sep = ",")
```
...
```{r}
load("Bases/Localidad/Indicadores_Escuelas_SEP.RData")
...
save(PC_SEP,  file = "Output/Localidad/PC_SEP.Rdata")
```
...
saveRDS(pcr_model, "Output/Localidad/pcr_model_sep.rds")
...
write.table(pcr_extremos, file = "Output/Localidad/PCR_SEP.txt", sep = ",")
```
...
```{r, class.source = "fold-show"}
load("Bases/Localidad/Indicadores_CLUES.RData")
...
save(PC_CLUES,  file = "Output/Localidad/PC_CLUES.Rdata")
```
...
saveRDS(pcr_model, "Output/Localidad/pcr_model_clues.rds")
...
write.table(pcr_extremos, file = "Output/Localidad/PCR_CLUES.txt", sep = ",")
```
...
```{r}
load("Bases/Localidad/IA_localidad.Rdata") 
```
...
```{r}
load("Output/Localidad/PC_IML.Rdata")
```
...
```{r}
load(file = "Bases/Localidad/Loc_Confidenciales_Claves.Rdata")
...
load("Output/Localidad/PC_DENUE.Rdata")
...
```{r}
load("Output/Localidad/PC_SEP.RData")
...
```{r}
load("Output/Localidad/PC_CLUES.RData")
...
```{r}
PCR_pred <- read.table("Output/Localidad/PCR_SEP.txt", header = TRUE, sep = ",") %>% 
             rbind(.,read.table("Output/Localidad/PCR_CLUES.txt", header = TRUE, sep = ",")) %>%
              rbind(., read.table("Output/Localidad/PCR_DENUE.txt", header = TRUE, sep = ","))
...
osé Bernardo Pena Trapero: `Problemas de la medición del bienestar y conceptos afines (1977)` 
...
save(sd, file = "Output/Localidad/sd.Rdata")
```

```{r, echo = FALSE}
load("Output/Localidad/sd.Rdata")
```
...
save(ICE_2020, file = "Bases/Localidad/ICE_2020.RData")
```
